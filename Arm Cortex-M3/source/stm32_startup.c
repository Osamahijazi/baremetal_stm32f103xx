...
#include<stdint.h> // include lib if used

#define SRAM_START   0x20000000U   
#define SRAM_SIZE   (20U * 1024U) //20k
#define SRAM_END    ((SRAM_START) + (SRAM_SIZE))

#define STACK_START   SRAM_END

// get boundries symbols fron linker script
...
//prototype of main func

int main(void);


// function prototypes of STM32F103xx system exception and IRQ handlers 
...
void Reset_Handler(void)
{
	...
	uint32_t size = (uint32_t)&_edata - (uint32_t)&_sdata;//Get size of data section by subtract both symbols
	
	uint8_t *pDst = (uint8_t*)&_sdata; //Destination pionter (Sram)
	
	uint8_t *pSrc = (uint8_t*)&_etext; //Source pionter (Flash)
	
	for(uint32_t i =0 ; i < size ; i++)
	{
		*pDst++ = *pSrc++;  // increment addresses to transfer Data
	}
	
	//Init. the .bss section to zero in SRAM	
	size = (uint32_t)&_ebss - (uint32_t)&_sbss; // get size of bss section
	
	pDst = (uint8_t*)&_sbss;  //Destination pionter (Sram)
	
	for(uint32_t i =0 ; i < size ; i++)
	{
		*pDst++ = 0; // Init all of them by Zero
	}
